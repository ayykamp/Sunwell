...
export default class SpellSchoolBanner extends Component {
	public assets(): string[] {
		return [this.parent.spellSchoolBannerAsset];
	}

	public render(context: CanvasRenderingContext2D, ratio: number) {
		if (!this.parent.spellSchoolBannerAsset || !this.parent.spellSchoolText) {
			return;
		}
		const coords = this.parent.spellSchoolBannerCoords;
		coords.ratio = ratio;
		const text = this.parent.spellSchoolText;

		// Draw the banner
		this.sunwell.drawImage(context, this.parent.spellSchoolBannerAsset, coords);
		...
		const textCoords = this.parent.spellSchoolTextCoords;
